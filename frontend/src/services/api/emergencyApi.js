...
import {
  requestEmergencyAccess as backendRequestEmergencyAccess,
  verifyEmergencyAccess as backendVerifyEmergencyAccess,
  endEmergencyAccess as backendEndEmergencyAccess,
} from "./backendAdapter";
...
export async function requestEmergencyAccess({ patientId, actorId, reason, ward, shift }) {
  if (isBackendEnabled()) {
    return backendRequestEmergencyAccess({ patientId, actorId, reason, ward, shift });
  }
...
export async function verifyEmergencyAccess({ grantId, code }) {
  if (isBackendEnabled()) {
    return backendVerifyEmergencyAccess({ grantId, code });
  }
  await new Promise((resolve) => setTimeout(resolve, 300));
...
export async function endEmergencyAccess({ grantId, actorId }) {
  if (isBackendEnabled()) {
    return backendEndEmergencyAccess({ grantId, actorId });
  }
